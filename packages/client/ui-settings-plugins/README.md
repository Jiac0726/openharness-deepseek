...
The first batch covers the shell executor (`bash`), browser-control blocked hosts (`browser-playwright-local`), the agent loop's tool-call parallelism (`agent-loop`), and the DeepSeek search provider (`web-search-deepseek`).
